...
- ตีความจากข้อมูลจริงที่ให้เท่านั้น เชื่อมโยงความหมายเชิงโหราศาสตร์ ต้องตรงกับข้อมูลที่ให้เป๊ะ (คำนวณให้สอดคล้องกับราศี/ตำแหน่งดาวจริงของคนนั้น ไม่สุ่มลอยๆ)
- สำคัญมาก: ให้บอกข้อมูลการทำนายที่ได้จริง ถ้าดวงดี ราบรื่น ก็บอกตามจริงว่าดวงดี ถ้าดวงไม่ดี ให้บอกตรงไปตรงมาว่าวันนี้มีความท้าทายจริงๆ ในเรื่องไหน ห้ามเปลี่ยนให้ฟังดูดีเกินจริงหรือกลบด้วยคำบวกจนเสียความหมายจริง แต่ให้ใช้คำพูดในเชิงค่อนข้างดีหน่อย เพื่อไม่ให้เสียกำลังใจ
- ตอบเป็นภาษาไทย สั้นๆ เข้าใจได้ง่าย แบ่งหัวข้อชัดเจน ขึ้นบรรทัดใหม่ทุกหัวข้อ ใส่อีโมจิหน้าหัวข้อให้เข้ากับเนื้อหา (ใช้ ⚠️ หรือ 🔻 กับหัวข้อที่ผลไม่ดี ไม่ต้องฝืนใช้อีโมจิบวกกับเรื่องลบ)
- แต่ละหัวข้อยาวไม่เกิน 2 ประโยค ห้ามเขียนพารากราฟยาว *สำคัญ ไม่ต้องบอกเกี่ยวกับ aspect/ดาว/การทำมุม
- ปิดท้ายทุกครั้งด้วย 3 บรรทัดนี้:
  🎨 สีมงคล: <สี ที่สอดคล้องกับธาตุ/ราศีของดาวเด่นในข้อมูล>
  🔢 เลขมงคล: <ตัวเลข 1-2 หลัก จำนวน 2 ตัวเลข >
  📌 คำแนะนำวันนี้: <คำแนะนำสั้นๆ ปฏิบัติได้จริง ถ้าดวงไม่ดีให้แนะนำวิธีรับมือ ใช้คำพูดดีๆหน่อย>
...
const MODE_PROMPTS = {
  overview: (u) => `ทำนายดวงภาพรวมจากข้อมูลตำแหน่งดาวจริงต่อไปนี้:\n\n${buildRealDataContext(u)}\n\nโครงสร้าง: 💫 ภาพรวม / 💼 การงาน / 💰 การเงิน / 💕 ความรัก / 💪 สุขภาพ แต่ละหัวข้อขึ้นบรรทัดใหม่ แล้วปิดท้ายด้วยสีมงคล/เลขมงคล/คำแนะนำ`,
  ...
  money: (u) => `ทำนายดวงการเงิน/การงานจากข้อมูลตำแหน่งดาวจริงต่อไปนี้ (เน้นดาวพฤหัสบดีและเสาร์):\n\n${buildRealDataContext(u)}\n\nโครงสร้าง: 💰 ภาพรวมการเงิน / 💼 การงาน / 📈 โอกาส แต่ละหัวข้อขึ้นบรรทัดใหม่ แล้วปิดท้ายด้วยสีมงคล/เลขมงคล/คำแนะนำ`,
  health: (u) => `ทำนายดวงสุขภาพจากข้อมูลตำแหน่งดาวจริงต่อไปนี้ (เน้นดาวเสาร์และดาวอังคาร):\n\n${buildRealDataContext(u)}\n\nโครงสร้าง: 💪 ภาพรวมสุขภาพ / ⚠️ ข้อควรระวัง แต่ละหัวข้อขึ้นบรรทัดใหม่ แล้วปิดท้ายด้วยสีมงคล/เลขมงคล/คำแนะนำ`,
  tarot: (u) => `สุ่มไพ่ทาโรต์ 1 ใบ ตีความให้เชื่อมโยงกับข้อมูลตำแหน่งดาวจริงต่อไปนี้:\n\n${buildRealDataContext(u)}\n\nโครงสร้าง: 🃏 ไพ่ที่สุ่มได้ (บอกชื่อไพ่) / 🔮 ความหมาย / ✨ สิ่งที่ไพ่ใบนี้บอกกับชีวิตช่วงนี้ แล้วปิดท้ายด้วยสีมงคล/เลขมงคล/คำแนะนำ`,
  numerology: (u) => `วิเคราะห์เลขศาสตร์จากวันเกิด ${u.birthdate} ประกอบกับข้อมูลตำแหน่งดาวจริงต่อไปนี้:\n\n${buildRealDataContext(u)}\n\nโครงสร้าง: 🔢 เลขชีวิต (คำนวณจากผลรวมวันเกิด) / 📖 ความหมายของเลขนี้ / 🌟 จุดแข็งที่ควรใช้ในช่วงนี้ แล้วปิดท้ายด้วยสีมงคล/เลขมงคล/คำแนะนำ`,
};

const CACHEABLE_MODES = new Set(["overview", "love", "money", "health","numerology"]); // tarot ไม่ cache เพราะควรสุ่มใหม่ทุกครั้งโดยธรรมชาติ
