...
- ตีความจากข้อมูลจริงที่ให้เท่านั้น เชื่อมโยงความหมายเชิงโหราศาสตร์ (เช่น เสาร์ตึงเครียดกับดาวส่วนตัว = บททดสอบ/ความรับผิดชอบ, ศุกร์ราบรื่นกับดวงจันทร์ = อารมณ์ดี ความสัมพันธ์ดี) แต่ตัวเลข/ตำแหน่ง/ชื่อ aspect ต้องตรงกับข้อมูลที่ให้เป๊ะ
- สำคัญมาก: ห้ามเข้าข้างว่าดวงต้องดีเสมอ ถ้าข้อมูล aspect ที่ให้มาเป็นมุมตึงเครียด (Square, Opposition) ให้บอกตรงไปตรงมาว่าวันนี้มีความท้าทายจริงๆ ในเรื่องไหน พร้อมเหตุผลจากดาวที่เกี่ยวข้อง ห้ามเปลี่ยนให้ฟังดูดีเกินจริงหรือกลบด้วยคำบวกจนเสียความหมายจริง ส่วนถ้า aspect เป็นมุมราบรื่น (Trine, Sextile) ก็บอกตามจริงว่าราบรื่นได้เต็มที่เช่นกัน
...
- ปิดท้ายทุกครั้งด้วย 3 บรรทัดนี้ (คำนวณให้สอดคล้องกับราศี/ตำแหน่งดาวจริงของคนนั้น ไม่สุ่มลอยๆ — ค่าพวกนี้ต้องนิ่ง ไม่เปลี่ยนไปมาถ้าข้อมูลตำแหน่งดาวเดิม):
  ...
  🔢 เลขมงคล: <ตัวเลข 2-3 หลัก>
  📌 คำแนะนำวันนี้: <คำแนะนำสั้น ปฏิบัติได้จริง อิงจาก aspect ที่ให้มา ถ้าดวงไม่ดีให้แนะนำวิธีรับมือ ไม่ใช่คำปลอบใจลอยๆ>
...
export async function generateDailyHoroscope(user) {
  ...
  const prompt = `ทำนายดวงประจำวันนี้ (${today}) จากข้อมูลตำแหน่งดาวจริงต่อไปนี้:
